...
class MyApp:
    ...
    def __init__(self, root):
        ...
        config = ConfigParser()
        config.read('C:\\Users\\Usuário\\AppData\\Local\\tgacotacao\\arquivo.ini')
...
class ConfigDB:
    def __init__(self, root):
        ...
        self.EntryPass.grid(padx=(20), pady=(20), row=1, column=2, sticky="nsew")
        self.Host = customtkinter.CTkLabel(master=root, text="Host: ")
        self.Host.grid(padx=(10), pady=(10), row=2, column=1, sticky="nsew")
        self.HostEntry = customtkinter.CTkEntry(master=root)
        self.HostEntry.insert(0, self.hostname)
        self.HostEntry.grid(padx=(10), pady=(10), row=2, column=2, sticky="nsew")
        ...
        self.conection.grid(padx=(10), pady=(10), row=0, column=3, sticky="nsew")

        self.save = customtkinter.CTkButton(master=root, text="Salvar", command=self.definirIni)
        self.save.grid(padx=(10), pady=(10), row=1, column=3, sticky="nsew")

        self.GetBack = customtkinter.CTkButton(master=root, text="Voltar", command=self.voltar)
        self.GetBack.grid(padx=(10), pady=(10), row=2, column=3, sticky="nsew")
    ...
    def connect(self):
        try:
            ...
            parametros_conexao = {
                'host': self.host,  # Você pode especificar o IP ou o nome do servidor Firebird
    ...
    def definirIni(self):
        ...
        path = self.CaminhoDB.get()
        user = self.EntryUser.get()
        password = self.EntryPass.get()

        FileIni().alterarIni(host, path, user, password)
...
class TelaInicial:
    def __init__(self, root, **kwargs):
        ...
        self.FunctionTitle = customtkinter.CTkLabel(self.root, text='Gerador de Link')
        self.FunctionTitle.grid(padx=(20), pady=(20), row=0, column=4, sticky="ew")
    ...
    def baixaExcel(self):
        ...
        new_archive = f'{values[0]} - {values[3]}'
...
class FileIni:
    ...
    def alterarIni(self, host, path, user, password):
        ...
        with open('C:\\Users\\Usuário\\AppData\\Local\\tgacotacao\\arquivo.ini', 'w') as configfile:
...
if __name__ == "__main__":
    ...
    app = MyApp(root)
    root.mainloop()
>>>>>>> b28c2232545ddef8990e80940f6580d68849c9b2
